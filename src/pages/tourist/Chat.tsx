...
import { MOCK_MESSAGES, MOCK_GUIDES } from '../../data/mockData';
...
export default function Chat() {
  const { user } = useAuth();
  const [activeChatId, setActiveChatId] = useState<string | null>('g1');
  const [messageInput, setMessageInput] = useState('');
  const [messages, setMessages] = useState(MOCK_MESSAGES);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Get unique conversation partners (guides for tourist, tourists for guide)
  // For demo, we'll assume current user is tourist 't1' and show chats with guides
  const conversations = Array.from(new Set(messages.map(m => m.senderId === 't1' ? m.receiverId : m.senderId)))
    .map(id => MOCK_GUIDES.find(g => g.id === id))
    .filter(Boolean);

  const activeConversation = MOCK_GUIDES.find(g => g.id === activeChatId);
  
  const currentMessages = messages.filter(
    m => (m.senderId === 't1' && m.receiverId === activeChatId) || 
         (m.senderId === activeChatId && m.receiverId === 't1')
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full flex">
        ...
        <div className="w-full md:w-80 border-r border-gray-100 flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.map(guide => (
              <button
                key={guide?.id}
                onClick={() => setActiveChatId(guide?.id || null)}
                className={`w-full p-4 flex items-center gap-3 hover:bg-gray-50 transition-colors ${
                  activeChatId === guide?.id ? 'bg-forest-600/5 border-r-4 border-forest-600' : ''
                }`}
              >
                <div className="relative">
                  <img
                ...
                <div className="flex-1 text-left min-w-0">
                  ...
                  <p className="text-sm text-gray-500 truncate">
                    {messages.findLast(m => m.senderId === guide?.id || m.receiverId === guide?.id)?.content || 'Start a conversation'}
                  </p>
                </div>
              </button>
            ))}
